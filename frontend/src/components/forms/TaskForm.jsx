...
export default function TaskForm({ onSubmit, onCancel, deals, companies, teamMembers = [], currentUser, taskTypes = TASK_TYPES, taskPriorities = TASK_PRIORITIES, dealStages = [], prefilledCompanyId = '' }) {
  ...
  return (
    <form className="form-grid" style={{ padding: '0 24px 24px' }} onSubmit={handleSubmit}>
      <label className="field">
        ...
        <input name="title" id="task-title-input" value={taskForm.title} onChange={handleChange} placeholder="Enter task title" required autoFocus />
      </label>

      <label className="field">
        <span>Type</span>
        <select name="type" id="task-type-select" value={taskForm.type} onChange={handleChange}>
          {taskTypes.map((t) => <option key={t} value={t}>{t}</option>)}
        </select>
      </label>

      <label className="field">
        <span>{canAssignSR ? 'Assign SR' : 'Owner'}</span>
        {canAssignSR ? (
          <select 
            name="owner" 
          ...
          >
            ...
            {availableSRs.map(m => (
              <option key={m.id} value={m.name}>{m.name} ({m.branch})</option>
            ))}
          </select>
        ) : (
          <input 
            name="owner" 
            id="task-owner-readonly"
            value={taskForm.owner} 
            readOnly 
      ...
      <label className="field">
        <span>Contact</span>
        <input name="contact" id="task-contact-input" value={taskForm.contact} onChange={handleChange} placeholder="Enter contact person" />
      </label>

      {/* Transferred Deal Fields */}
      <label className="field">
        <span>Deal stage</span>
        <select name="dealStage" id="task-deal-stage-select" value={taskForm.dealStage} onChange={handleChange}>
      ...
      <label className="field">
        ...
        <input name="dealValue" id="task-deal-value-input" type="number" min="0" value={taskForm.dealValue} onChange={handleChange} placeholder="Enter value" />
      </label>

      <label className="field">
        <span>Expected close</span>
        <input name="expectedClose" id="task-expected-close-input" type="date" value={taskForm.expectedClose} onChange={handleChange} />
      </label>

      <label className="field">
        <span>Due date</span>
        <input name="dueDate" id="task-due-date-input" type="date" value={taskForm.dueDate} onChange={handleChange} required />
      </label>

      <label className="field">
        <span>Priority</span>
        <select name="priority" id="task-priority-select" value={taskForm.priority} onChange={handleChange}>
